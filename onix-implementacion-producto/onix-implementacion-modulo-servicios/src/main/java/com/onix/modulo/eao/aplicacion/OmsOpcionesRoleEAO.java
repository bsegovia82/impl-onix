...
public class OmsOpcionesRoleEAO extends OnixEAO<OmsOpcionesRole, Long> {
	...
	public OmsOpcionesRole obtenerOpcionRol(Integer idOpcion, Integer idRol){
		Query query = adminEntidad.createNativeQuery("select op.* from OMS_OPCIONES_ROLES op "+
				" where op.id_opcion = ? "+
				" and op.id_rol = ? ", OmsOpcionesRole.class);
	...
	public void actualizarOpcionesRol(String estado, Integer idOpRol){
		String lsQuery = " update OMS_OPCIONES_ROLES b "+
				" set b.estado = ?, b.fecha_actualizacion=sysdate "+
				" where b.id = ? ";
	...
	public void eliminarOpcionesRol(Long idRol){
		String lsQuery = " delete OMS_OPCIONES_ROLES b "+
				" where b.ID_ROL = :idRol ";
		Query query =  adminEntidad.createNativeQuery(lsQuery);
	...
	public List<OmsOpcione> obtenerListaOpcionesEjecutablesRol 
	(
			Long idRol, String tipo)
	{  
		String lsQuery = " select f.* from oms_opciones f, oms_opciones_roles s "
				+ "where f.id = s.id_opcion "
				+ "and f.estado = 'A' "
				+ "and f.accion is not null "
				+"and f.tipo = :tipo "
				+ "and s.id_rol = :idROl";
